...
void Redirected_Post_Data_Update(void* Entity, void* Unknown_Parameter)
{
	...
	if (Simulation_Time != Previous_Simulation_Time)
	{
		...
		Player_Data->Teleported = __builtin_powf(Origin[0] - Player_Data->Origin[0], 2.f) + __builtin_powf(Origin[1] - Player_Data->Origin[1], 2.f) + __builtin_powf(Origin[2] - Player_Data->Origin[2], 2.f) > 4096.f;
