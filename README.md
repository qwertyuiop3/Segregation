...
alias -conditional_jump "exec stand_condition;-jump"
...
|`Minimum_Choked_Commands`|Минимум задушенных команд|[0, 21]|
|`Maximum_Choked_Commands`|Максимум задушенных команд|[0, 21]|
...
|`Angle_X`|Угол вертикального наклона головы, ограничивается диапозоном [-90, 90] в серверном [`FinishMove`](https://github.com/ValveSoftware/source-sdk-2013/blob/master/src/game/server/player_command.cpp#L219-L225), но искажается перед этим нормализацией клиентской интерполяции.<br>Компрессия чисел на `Windows` отличается, тем самым искажая значение с `180.0` до `180.08794` при получении клиентом.|[-180.0, 180.0]|
