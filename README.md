...
Чтобы выровнять, насколько то возможно, снимок данных (углы, координаты, анимационные циклы) цели во время компенсаторики RTT в [интерполяционной доле](https://github.com/rom4s/hl2sdk-ep1c/blob/game-cstrike-1/game/server/player_lagcompensation.cpp#L471-L473), что высчитывается при негативном расхождении между серверным и клиентским штампом времени обновления, и минимизировать сдвиг (ограниченный в ~200ms, если не учитывать `sv_maxunlag` - ответственный за [компенсацию RTT](https://github.com/rom4s/hl2sdk-ep1c/blob/game-cstrike-1/game/server/player_lagcompensation.cpp#L335-L359)) от последнего штампа, таким образом продлевая окно попадания: `cl_updaterate N;cl_interp_ratio 0;cl_interp 0;host_writeconfig`, где `N` подразумевает частоту симуляции конкретного сервера.  
