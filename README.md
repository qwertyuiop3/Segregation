...
Чтобы выровнять, насколько то возможно, снимок данных (углы, координаты, анимационные циклы) цели во время компенсаторики RTT в [интерполяционной доле](https://github.com/ValveSoftware/source-sdk-2013/blob/master/src/game/server/player_lagcompensation.cpp#L519-L521), что высчитывается при негативном расхождении между серверным и клиентским штампом времени обновления, и минимизировать сдвиг (ограниченный в ~200ms, если не учитывать `sv_maxunlag` - ответственный за компенсацию RTT) от последнего штампа, тем самым продлевая окно попадания: `cl_updaterate N;cl_interp_ratio 0;cl_interp 0;host_writeconfig`, где `N` подразумевает частоту симуляции конкретного сервера.  
...
```
alias +segregation_jump "exec apply_segregation_jump;+jump"
alias -segregation_jump "exec revert_segregation_jump;-jump"
bind space +segregation_jump
```
...
|`Angle_X`|Угол вертикального наклона головы|[-180.0, 180.0]|
...
|`Angle_Y`|Передаваемый угол горизонтального поворота тела|[-180.0, 180.0]|
