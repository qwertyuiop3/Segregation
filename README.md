...
|`Bruteforce`|Повыстрельное нахождение (коллидирующего) `m_flGoalFeetYaw` методом "грубой силы".<br>Выстрел устанавливает последующий целевой `m_flGoalFeetYaw` и входит в режим ожидания события.<br>По пришествию `player_hurt` либо `player_death`, фиксируется угол по состоянию попадания, и устанавливаются последующие терпимости.<br>По пришествию `bullet_impact` в течении ~500ms, выстрел классифицируется зарегистрированным, в противном случае подборочный снимок данных откатывается до предвыстрельного.|[0, 1]|
...
|`Duck_Advantage`|Прожатие `+duck` в случае преимущества горизонтальной скорости против `-duck`|[0.0, 3.4028235e38 - Speed]|
