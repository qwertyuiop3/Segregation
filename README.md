...
|`Target_On_Simulation`|Требование к ожиданию обновления `m_flSimulationTime` перед выстрелом, - в придачу сломления [условия телепортации](https://github.com/rom4s/hl2sdk-ep1c/blob/game-cstrike-1/game/server/player_lagcompensation.cpp#L431-L436).<br>В первом случае возможно искажение [относительных операций](https://github.com/rom4s/hl2sdk-ep1c/blob/game-cstrike-1/game/client/c_baseentity.cpp#L362-L368); во втором же возможен ложноотрицательный либо ложноположительный результат по причине отсутствия истинных промежуточных.|[0, 2]|
...
|`Angle_X`|Угол вертикального наклона головы.<br>Ограничивается диапазоном [-90.0, 90.0] в серверном [`FinishMove`](https://github.com/rom4s/hl2sdk-ep1c/blob/game-cstrike-1/game/server/player_command.cpp#L208-L222), однако результирующее искажается получателем, а точнее, нормализацией значений, последующей клиентской интерполяцией.<br>Компрессия чисел на `Windows` отличается, тем самым искажая значение с `180.0` до `180.08794` при получении клиентом.|[-180.0, 180.0]|
